...
from sqlalchemy import create_engine, VARCHAR, TIMESTAMP, TEXT, text
...
class GitWatcher():
    ...
    - last_page_num
    """
    def _parse_header(self, _r):
        last_modified = self._datetime_from_string(_r.headers['Last-Modified'])
        # if less than 100 items there is no 'Link' in _r headers
        if 'Link' in _r.headers:
            pages = _r.headers['Link'].split(", ")
    ...
    def download(self):
        ...
        if response.status_code != 304:
            ...
            # export to DB
            engine = create_engine(self.DB_URI, client_encoding='utf8')
...
def watcher_task():
    ...
    app_config['DB_URI'] = os.environ.get('DATABASE_URL')
    # app_config['DB_URI'] = "postgres://primaryuser:password@localhost:5432/analytics_dev"
    ...
    for (idx, _r) in enumerate(repos.values):
        ...
        # update last_update if changed
        if _n != _r[4]:
            query = "UPDATE control_repos SET last_update = '{}' WHERE url = '{}'".\
                format(_n, _r[3])
            engine.execute(text(query))
            print("Updating {} with {}".format(_r[3], _n))
