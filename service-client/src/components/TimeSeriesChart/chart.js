...
class TimeSeriesChart extends Component {
  ...
  _onMouseMove(e) {
    ...
    let bisectDate = bisector(function(d) { return d.date; }).right;
    ...
    try {
      ...
      yReadings = _.pick(data[dataIdx], coins)
    }
    catch(err) {
      hoverActive = false
    }
  ...
  render() {
    ...
    const coins = Object.keys(data[0]).slice(1)
    const dMax = (d) => coins.reduce((a,b) => a > d[b] ? a : d[b], 0);
